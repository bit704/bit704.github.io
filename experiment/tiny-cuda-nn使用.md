...
Github: [ tiny-cuda-nn](https://github.com/NVlabs/tiny-cuda-nn)
...
(c)每个线程块进行矩阵乘法运算，将第i层$H_{i}$转换为预先激活的下一层$H^{'}_{i+1}$。它被切成16 × 16个元素的块，以匹配我们的硬件加速的半精度矩阵乘法器(TensorCore)的大小。每个线程块的扭曲计算一个16 × 128块行$H^{'}_{i+1}$(例如条带区域)，首先从$W_i$加载相应的16 × 64条带权重到寄存器，然后将它们乘以$H_{i}$的所有64 × 16块列。因此，每个线程块只从全局内存加载一次权值矩阵(尽可能少的量)，只在位于快速共享内存中的Hi上进行多次传递。
